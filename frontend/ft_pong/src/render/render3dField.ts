...
import { setStarDestroyerReferences, setBackgroundReferences } from './debug3d';
...
export function addBackgroundImage(scene: BABYLON.Scene, imageName: string) {
  ...
  const tex = new BABYLON.Texture(`/ft_pong/public/models/${imageName}`, scene, false, false, BABYLON.Texture.TRILINEAR_SAMPLINGMODE, 
    undefined, 
...
export async function loadStarDestroyerBackground(scene: BABYLON.Scene) {
  ...
  const starDestroyerTask = assetsManager.addMeshTask('starDestroyer', '', '/ft_pong/public/models/', 'star_destroyer.glb');
