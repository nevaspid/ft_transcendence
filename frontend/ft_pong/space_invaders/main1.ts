...
function showVictoryOverlay(
  ...
  over.innerHTML = `
    <div class="si-card">
      ...
      ` : `
        <p>Score: ${score}</p>
        <p>Vies ${pseudoUser}: ${p1?.lives ?? 0}${p2 ? ` | Vies P2: ${p2.lives}` : ''}</p>
      `}
...
function showDefeatOverlay(wrapper: HTMLElement, score: number, p1: Ship | null, p2: Ship | null, onRetry: () => void): void {
  ...
  over.innerHTML = `
    <div class="si-card">
      <h2>Défaite</h2>
      <p>Score: ${score}</p>
      <p>Vies ${pseudoUser}: ${p1?.lives ?? 0}${p2 ? ` | Vies ${p2Name}: ${p2.lives}` : ''}</p>
      <div class="si-actions">
        <button id=\"si-retry\" class=\"si-btn\">Rejouer</button>
        <a href=\"menu.html\" class=\"si-btn\" style=\"text-decoration:none;display:inline-block\">Menu</a>
      </div>
...
function promptP2Name(): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    over.innerHTML = `
      <div class="si-card">
        <h2>Coop: pseudos</h2>
        <div style="margin:8px 0">Joueur 1: <strong>${pseudoUser}</strong></div>
